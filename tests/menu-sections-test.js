...
// Test 2: fixed, predictable section order with the catch-all last
{
  ...
  const expected = ['bourbon', 'scotch', 'gin', 'rum', 'agave', 'vodka', 'brandy', 'other'];
...
// Test 3: a drink goes where its first (dominant) spirit tag says
{
  ...
  assert.equal(sectionKeyFor(drink('b', ['classic', 'whiskey-forward', 'rum-forward'])), 'bourbon');
  assert.equal(sectionKeyFor(drink('b2', ['classic', 'scotch-forward'])), 'scotch');
...
// Test 4: headings only when a menu is long enough to need them
{
  ...
  const oneSpirit = SEED_RECIPES.filter(r => sectionKeyFor(r) === 'bourbon').slice(0, 30);
...
// Test 5: the sizes are sane on the real library (guards against a tag change emptying a section)
{
  ...
  assert.ok(size.bourbon >= 30 && size.gin >= 30 && size.rum >= 30, `Big three should be big: ${JSON.stringify(size)}`);
  assert.ok(size.scotch >= 5, `Scotch should have a dedicated presence: ${size.scotch}`);
