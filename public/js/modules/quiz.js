...
// so each band is a real slice of a menu.
...
const VIBES = [
  ...
  { value: 'savory', label: 'Savory', hint: 'Olive, pickle, salty, umami', tags: ['savory'] },
];
...
export const SPIRITS = [
  { value: 'bourbon', label: 'Bourbon', heading: 'Bourbon', tags: ['bourbon-forward', 'whiskey-forward'] },
  { value: 'scotch', label: 'Scotch', heading: 'Scotch', tags: ['scotch-forward'] },
...
// Weights: a spirit or vibe someone names outranks a strength
...
const WEIGHTS = { vibe: 4, spirit: 4, strengthExact: 2, strengthNear: 1 };

function matchesVibe(vibe, t) {
  return (vibe.moods && vibe.moods.some(m => t.moods.includes(m)))
    || (vibe.tags && vibe.tags.some(tag => t.tags.has(tag)));
}
...
export function buildQuizQuestions(recipes, traits) {
  ...
  const vibes = VIBES.filter(v => has(t => matchesVibe(v, t)));
